...
async function runTransportSuite(label, makeDb) {
    ...
    await test("selectGt strings with wire-specials (regression)", async () => {
        // Mirror of Python test_selgt_strings_with_wire_specials.
        // Pins all five escape cases: , ; \ CR LF.
        const h2 = await db.open();
        const t2 = await db.create(h2, "specials", [
            ["name",  2],   // STRING
            ["score", 0],   // INT
        ]);
        const rowsIn = [
        ...
        for (const r of rowsIn) await db.insert(h2, t2, r);
        const out = await db.selectGt(h2, t2, 1, 5);
        assert.equal(out.length, rowsIn.length,
            `row count split: got ${out.length} want ${rowsIn.length}`);
        for (const r of out) {
            assert.equal(r.length, 2,
                `column count split: got ${r.length} want 2 — row=${JSON.stringify(r)}`);
        }
        // Values match: build a {score: name} map and compare.
        const byScore = Object.fromEntries(out.map(r => [parseInt(r[1], 10), r[0]]));
        for (const [name, score] of rowsIn) {
            assert.equal(byScore[score], name,
                `mismatch at score=${score}: got ${byScore[score]} want ${name}`);
        }
    });

    await test("get round-trips strings with wire-specials (regression)", async () => {
        // Closes the gap that no test exercised GET with ; ,  \ CR LF.
        // wire_str_encode now escapes all five; splitWireRow decodes.
        const h2 = await db.open();
        const t2 = await db.create(h2, "get_specials", [["name", 2]]);
        const specials = ["plain", "co,mma", "se;mi", "back\\slash",
                          "cr\rcr", "lf\nlf", "all,;\\\r\n!"];
        for (let i = 0; i < specials.length; i++) {
            await db.insert(h2, t2, [specials[i]]);
            const row = await db.get(h2, t2, i);
            assert.deepEqual(row, [specials[i]],
                `GET round-trip failed: stored=${JSON.stringify(specials[i])} got=${JSON.stringify(row[0])}`);
        }
    });
