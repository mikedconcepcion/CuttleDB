...
|----------|--------------------|-------|
| 0.9.x    | :white_check_mark: | Current. Security fixes land on patch releases. |
| ≤ 0.8.x  | :x:                | Superseded — upgrade to 0.9.x. |

CuttleDB follows [Semantic Versioning](https://semver.org/). Security
patches land as patch-version bumps (`0.9.x → 0.9.(x+1)`) within the
...
expanding; v1.0 ships when graph types, distributed sync, and
temporal queries all land.
